"""
Download triangles.pkl and circles.pkl before running.
The decision boundary is obtained using bayes classifier.
"""
import pickle
import converter
import numpy as np
...
for i in range(0,10):
    ...
    tri=plt.scatter(triangles[i][0], triangles[i][1], color="g",marker='o')
utri=plt.scatter(mean1[0],mean1[1],color="y",marker='+')
for i in range(0,10):
    print(circles[i][0],circles[i][1])
    cir=plt.scatter(circles[i][0], circles[i][1], color="r",marker='o')
ucir=plt.scatter(mean2[0],mean2[1],color="b",marker='+')
